/*************************************************************************
> FileName: gio-resolver.c
> Author  : DingJing
> Mail    : dingjing@live.cn
> Created Time: Tue 14 Mar 2023 05:37:46 PM CST
 ************************************************************************/
#include <glib.h>
#include <stdio.h>
#include <stdlib.h>
...
static void do_async_connectable (GSocketAddressEnumerator *enumerator);
static void do_connectable (const char *arg, gboolean synch, guint count);
...
static void print_resolved_ns (const char* rrname, GList* records, GError* error);
static void print_resolved_mx (const char* rrname, GList* records, GError* error);
static void print_resolved_txt (const char* rrname, GList* records, GError* error);
static void print_resolved_srv (const char* rrname, GList* records, GError* error);
...
static void lookup_records_callback (GObject* source, GAsyncResult *result, gpointer user_data);
...
static void lookup_by_addr_callback (GObject *source, GAsyncResult *result, gpointer user_data);
static gboolean record_type_arg (const gchar *option_name, const gchar *value, gpointer data, GError **error);
...
static void usage (void)
{
    fprintf (stderr, "Usage: gio-resolver [-s] [hostname | IP | service/protocol/domain ] ...\n");
    fprintf (stderr, "Usage: gio-resolver [-s] [-t MX|TXT|NS|SOA|SRV] rrname ...\n");
    fprintf (stderr, "       gio-resolver [-s] -c NUMBER [hostname | IP | service/protocol/domain ]\n");
...
static void print_resolved_soa (const char* rrname, GList* records, GError* error)
{
    GList *t;
...
static void start_async_lookups (char **argv, int argc)
{
    ...
    for (i = 0; i < argc; i++) {
        if (record_type != 0) {
            g_resolver_lookup_records_async (resolver, argv[i], record_type, cancellable, lookup_records_callback, argv[i]);
        }
            ...
            g_resolver_lookup_by_name_async (resolver, argv[i], cancellable, lookup_by_name_callback, argv[i]);
        }
