/*************************************************************************
> FileName: test-fm-dec-ipc.c
> Author  : DingJing
> Mail    : dingjing@live.cn
> Created Time: 2025年04月21日 星期一 16时04分05秒
 ************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define IPC_SERVER_SOCKET_PATH              "/usr/local/andsec/start/andsec_status.sock"
...
int main (int argc, char* argv[])
{
	...
	send_data_to_daemon(6, argv[1], strlen(argv[1]), NULL, 0);
...
int send_data_to_daemon(unsigned int type, const char* sendData, int sendDataLen, char* recvData, int recvDataBufLen)
{
    ...
    do {
        ...
        msgBuf.type = type;
        msgBuf.dataLen = sendDataLen + sizeof(dd);
		dd.dataLen = sendDataLen;

        // 开始发送
        int allLen = (int) sizeof(struct IpcMessage) + sendDataLen + sizeof(struct D);
        ...
        if (sendDataLen > 0) {
            memcpy(sendBuf + sizeof(msgBuf), &dd, sizeof(dd));
            memcpy(sendBuf + sizeof(msgBuf) + sizeof(dd), sendData, sendDataLen);
        }
