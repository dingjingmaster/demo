...
[[ ! -d $sysroot/lib64 ]] && cd $sysroot && ln -s usr/lib lib64 && cd -
...
case $(uname -m) in
    ...
    ;;
    x86_64) ln -sfv ../lib/ld-linux-x86-64.so.2 $sysroot/lib64/ld-lsb-x86-64.so.3
    ;;
esac
cd -
...
../configure \
    ...
    --disable-nscd                      \
    CC=$gccBase/bin/$target-gcc         \
    CXX=$gccBase/bin/$target-g++        \
